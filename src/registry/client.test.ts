...
describe("splitImageName", () => {
  const cases: Array<{ name: string; registry: string; repository: string }> = [
...
describe("RegistryClient.resolveDigest", () => {
  ...
  it("rejects registries on non-public addresses", async () => {
    ...
    for (const image of [
      ...
      await assert.rejects(
        () => client.resolveDigest(image, "latest"),
        /non-public address/,
        image
      );
    }
  ...
  it("does not trust a cross-origin token realm", async () => {
    const { url, close } = await startServer((_req, res) => {
      res.writeHead(401, {
        "WWW-Authenticate":
          'Bearer realm="https://169.254.169.254/token",service="registry.example"',
      });
      res.end();
    });
    ...
    try {
      ...
      await assert.rejects(
        () => client.resolveDigest("registry.example/team/app", "latest"),
        /non-public address/
      );
    } finally {
      close();
