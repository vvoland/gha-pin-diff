...
function resolvesTo(digest: string) {
...
function imageUpdate(over: Partial<ActionUpdate> = {}): ActionUpdate {
  return {
...
function clientFor(url: string): RegistryClient {
...
describe("check", () => {
  ...
  it("deduplicates registry calls for the same image and tag", async () => {
    let calls = 0;
    const { url, close } = await startServer((req, res) => {
      if (req.url?.includes("/manifests/")) calls++;
