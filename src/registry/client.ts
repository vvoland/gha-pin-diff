...
export class RegistryError extends Error {
  ...
  constructor(method: string, url: string, statusCode: number, detail = "") {
...
export interface ImageName {
  registry: string; // API host, e.g. "registry-1.docker.io" or "ghcr.io"
  repository: string; // e.g. "library/nginx" or "owner/repo"
}
 ...
 */
export function splitImageName(name: string): ImageName {
...
export class RegistryClient {
   ...
   */
  async resolveDigest(name: string, tag: string): Promise<string> {
  ...
  private async authenticate(
    ...
    if (!challenge) throw new Error("missing WWW-Authenticate challenge");

    const params = parseChallenge(challenge);
    const realm = params.get("realm");
    if (!realm) throw new Error("WWW-Authenticate challenge has no realm");
...
async function resolvePublicAddresses(
  ...
  for (const address of addresses) {
    ...
    if (blockedAddresses.check(address.address, type)) {
      throw new Error(
        `registry host resolves to a non-public address: ${address.address}`
      );
    }
  }
  return addresses;
...
function parseChallenge(challenge: string): Map<string, string> {
