...
import { scan, type Pin } from "./scanner.js";
    ...
    promises.push(
      (async () => {
        ...
        try {
          ...
          console.warn(
            `warning: could not resolve tag ${pin.tag} for ${pin.action}: ${err}`
          );
        }
      })()
    );
...
export async function run(args: string[]): Promise<void> {
  ...
  for (const m of mismatches) {
    console.log(
      `  ${m.file}:${m.line}: ${m.action} pinned to ${m.sha.substring(0, 7)} but ${m.tag} resolves to ${m.expectSHA.substring(0, 7)}`
    );
  }
  console.log("");
  process.exit(1);
